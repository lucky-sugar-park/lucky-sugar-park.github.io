...
1. Big bang 방식의 전환 vs 점증적 전환
   - 새로운 Frontend 개발과 테스트를 완료한 후에, 기존 Frontend를 일시에 대체할 것인가? 아니면 기존 Frontend를 점증적으로 새로운 Frontend로 옮길 것인가? 
   - Big Bang 방식 / 점증적 방식 모두 장단점이 있기 때문에, 조직의 상황에 맞는 방식을 선택해야 한다.
   - 몇 달 정도의 단시간 내에 전환할 목적이라면 Big Bang 방식적용이 유리할 수 있다.
2. 동일한 기능을 제공하는 Frontend에 대해서 One Source 정책을 유지할 필요가 있는가?
   - 두 가지 방식 모두, (신규기능추가 또는 기존 기능 업데이트 등의) 요구가 발생할 때에, 신규기능에 반영하고 기존 Frontend 를 수정할 것인가? 아니면 요구되는 신규기능을 새로운 Frontend로 개발하고 개발된 Frontend를 기존 Frontend와 통합할 것인가?
   - 기존 Frontend에 Micro Frontend 아키텍처를 적용함으로써 One Source 정책 유지가 가능할 수 있으나, 매우 어렵거나 불가능할 수도 있다
   - 인력과 시간이 충분하다면, 전환과정에서의 Multi Source 정책을 선택하는 것이 안정적일 수 있다.
3. 기존 운영중인 Legacy Frontend에 대해서 (사용자들 서비스 향상을 위해) 지속적인 기능 향상을 제공할 필요가 있는가?
   - Big Bang이 아닌, 점증적 전환정책을 선택할 경우에는 전환완료까지 오랜 시간이 소요될 수 있기 때문에 기존 Frontend에 대한 기능향상이나 신규기능 추가를 고려해야 한다.
4. Legacy Frontend에 대한 업데이트 유지보수 인력이 충분한가?
   - 인력이 충분하다면 Big Bang 방식을 고려해볼 만 하다
