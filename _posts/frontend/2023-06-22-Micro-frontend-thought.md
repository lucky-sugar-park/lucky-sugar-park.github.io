...
## 백엔드 통신  
프런트엔드 어플리케이션에서 독립적으로 작업하는 별도의 팀이 있다면 백엔드 개발은 어떻하는 것이 좋을까? 하나의 팀에서 UI부터 API 개발, 데이터베이스 및 인프라 코드에 이르기까지 전체를 담당하는 것을 강력하게 권장한다. 여기서 도움이 되는 한 가지 패턴은 BFF 패턴이다. 여기서 각 프런트엔드 애플리케이션에는 해당 프런트엔드의 요구 사항을 충족하는 것이 목적인 해당 백엔드가 있다. BFF 패턴은 원래 각 프런트엔드 채널(웹, 모바일 등)에 대한 전용 백엔드를 의미했을 수 있지만 각 마이크로 프런트엔드에 대한 백엔드로 쉽게 확장할 수 있다.  
